...
const STATUS_MESSAGES: Record<DossierStatut, string> = {
  ...
  en_etude: 'Notre équipe examine actuellement votre dossier et prépare votre itinéraire ou votre étude visa. Vous recevrez notre réponse très prochainement.',
  confirme: 'Excellent ! Votre voyage est confirmé. Notre équipe va vous contacter pour finaliser les détails et le paiement.',
  paye: 'Votre dossier est complet et votre voyage est entièrement organisé. Bon voyage !',
  termine: 'Nous espérons que votre voyage s\'est parfaitement déroulé ! N\'hésitez pas à nous laisser un avis.',
  annule: 'Votre dossier a été annulé. Pour toute question, contactez-nous sur WhatsApp.',
}

export async function POST(req: NextRequest) {
  try {
    const { dossier_id, nouveau_statut } = await req.json() as { dossier_id: string; nouveau_statut: DossierStatut }
    ...
    const { data: profile } = await supabase
      .from('profiles').select('full_name').eq('id', dossier.client_id).single()

    const { data: user } = await supabase.auth.admin.getUserById(dossier.client_id)
