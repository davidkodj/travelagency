...
export default async function PrivacyPage({ params: { locale } }: Props) {
  ...
  const { data: page } = await supabase.from('pages_contenu').select('contenu,updated_at').eq('slug', 'privacy').single()
  const DEFAULT = 'Politique de confidentialité non encore configurée. Connectez-vous en tant qu\'admin pour la configurer.'
    ...
    <>
      <div className="min-h-screen bg-abyss pt-16">
        <div className="container max-w-3xl px-6 mx-auto py-14">
          ...
          <h1 className="font-display font-bold text-4xl text-ivory mb-3">Politique de confidentialité</h1>
