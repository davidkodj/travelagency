        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 flex flex-col gap-5">
            <Card className="bg-raised border-subtle">
              ...
              <CardContent className="pt-5 grid grid-cols-2 gap-4 text-sm">
                {[
                  ...
                  <div key={k}>
                    <div className="text-xs text-ivory-muted uppercase tracking-wider mb-1">
                      {k}
                    </div>
                    <div className="font-medium text-ivory capitalize">
                      {v ?? "—"}
                    </div>
                  </div>
                ))}
            ...
            <Card className="bg-raised border-subtle">
              ...
              <CardContent className="pt-5">
                <p className="text-sm text-ivory-muted leading-[1.8] whitespace-pre-wrap">
                  {dossier.message_client || "Aucun message."}
                </p>
              </CardContent>
            </Card>
              ...
              <Card className="bg-raised border-subtle">
                ...
                <CardContent className="pt-5">
                  <div className="flex flex-col gap-4">
                    {historique.map((h) => {
                      const from = STATUTS.find(
                        (s) => s.value === h.ancien_statut,
                      );
                      const to = STATUTS.find(
                        (s) => s.value === h.nouveau_statut,
                      );
                      return (
                        <div key={h.id} className="flex gap-3 items-start">
                          ...
                          <div className="text-sm">
                            <div className="flex items-center gap-2 flex-wrap">
                              <span
                                className={`text-xs px-2 py-0.5 rounded-full border ${from?.cls ?? ""}`}
                              >
                                {from?.label ?? h.ancien_statut}
                              </span>
                              <ChevronRight className="w-3 h-3 text-ivory-muted" />
                              <span
                                className={`text-xs px-2 py-0.5 rounded-full border ${to?.cls ?? ""}`}
                              >
                                {to?.label ?? h.nouveau_statut}
                              </span>
                            </div>
                            {h.commentaire && (
                            ...
                            <p className="text-xs text-ivory-muted/50 mt-0.5">
          ...
          <div className="flex flex-col gap-5">
            <Card className="bg-raised border-subtle">
              ...
              <CardContent className="pt-4 flex flex-col gap-2">
                ...
                <div className="mt-2 space-y-2">
                  <Label className="text-ivory-muted">
            ...
            <Card className="bg-raised border-subtle">
              ...
              <CardContent className="pt-4 flex flex-col gap-3">
                ...
                <Button
                  onClick={save}
                  disabled={saving}
                  variant={saved ? "secondary" : "copper"}
            ...
            {dossier.profiles?.phone && (
              <a
                href={`https://wa.me/${dossier.profiles.phone.replace(/\D/g, "")}`}
                target="_blank"
