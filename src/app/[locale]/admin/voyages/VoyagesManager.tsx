...
export default function VoyagesManager({ locale, initialVoyages }: Props) {
  ...
  const onSubmit = async (data: F) => {
    ...
    if (editing) {
      ...
      if (!error && updated) {
        setVoyages((v) =>
          v.map((x) => (x.id === editing.id ? (updated as Voyage) : x)),
        );
        setOpen(false);
      }
    } else {
      const { data: created, error } = await supabase
        ...
        .single();
      if (!error && created) {
        setVoyages((v) => [created as Voyage, ...v]);
        setOpen(false);
      }
    }
  };
    ...
    <div className="min-h-screen bg-abyss pt-16">
      <div className="container max-w-5xl px-6 mx-auto py-12">
        ...
        <div className="flex items-center justify-between mb-10">
          ...
          <Button variant="copper" onClick={openCreate} className="gap-2">
        ...
        {voyages.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {voyages.map((v) => (
              <div
                key={v.id}
                className="group bg-raised border border-subtle rounded-2xl overflow-hidden hover:border-copper/25 transition-colors"
              >
                ...
                <div className="p-5 flex flex-col gap-2.5">
                  <h3 className="font-display font-semibold text-ivory leading-tight">
                    {v.titre}
                  </h3>
                  <p className="text-xs text-ivory-muted">
                    {v.destination} — {v.duree_jours}j
                  </p>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-subtle">
                    <span className="font-display font-bold gradient-text">
                      ...
                      <span className="text-xs text-ivory-muted font-normal">
                        EUR
                      </span>
                    </span>
                    <div className="flex gap-1">
                      <button
                        onClick={() => openEdit(v)}
                        className="w-8 h-8 flex items-center justify-center rounded-lg text-ivory-muted hover:text-copper hover:bg-copper/10 transition-all"
                      >
            ...
            >
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2 space-y-2">
                  ...
                  <Input
                    {...register("titre")}
                    placeholder="Circuit Europe Classique"
                  />
                  {errors.titre && (
                ...
                <div className="col-span-2 flex items-center gap-3">
                  ...
                  <Label
                    htmlFor="actif"
                    className="cursor-pointer normal-case text-sm text-ivory-dim"
                  >
                    Voyage actif (visible sur le site)
                  </Label>
                </div>
              </div>
